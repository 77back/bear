...
import { applyResult } from '@shared/cards'
import { fetchJson } from '@/platform/request'

/**
 * 刷题卡片 store（与网页版 stores/cards.ts 行为一致）。
 * 卡片静态 JSON 从内容服务器（imback.icu）按需加载；
 * 答题记录与掌握状态存本地 storage（见 db/index.ts），离线可用。
 */
...
export const useCardsStore = defineStore('cards', () => {
  ...
  async function loadIndex(): Promise<void> {
    try {
      index.value = await fetchJson<CardsIndexEntry[]>('cards/index.json')
    } catch {
      index.value = []
    }
  }
  ...
  async function loadDeck(key: string): Promise<Card[]> {
    ...
    if (hit) return hit
    try {
      const cards = await fetchJson<Card[]>(`cards/cards-${key}.json`)
  ...
  async function recordAttempt(
    ...
    // 缓存取出的是 Vue 响应式 Proxy，展开为纯对象再累计
