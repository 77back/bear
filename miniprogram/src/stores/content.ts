...
/**
 * 内容 store（与网页版 stores/content.ts 行为一致）。
 * 从内容服务器拉取管线产出；platform/request 自带 NetworkFirst 缓存，
 * 断网回退到最近一次内容。
 */
...
// 案例归档（archive/cases.json，管线累积产出）
...
// 每日纠错知识点（shiwu/knowledge.json，静态题库，按日轮换）
...
// 媒体备考板块（media/*.json，静态人工维护；文件缺失优雅降级为空数组）
...
export const useContentStore = defineStore('content', () => {
  ...
  async function loadArchive(): Promise<void> {
    try {
      archive.value = await fetchJson<ArchiveCase[]>('archive/cases.json')
    } catch {
      archive.value = []
    }
  }
  ...
  async function loadMedia(): Promise<void> {
    try {
      mediaOrgs.value = await fetchJson<MediaOrg[]>('media/orgs.json')
    } catch {
      mediaOrgs.value = []
    }
    try {
      mediaKnowledge.value = await fetchJson<MediaKnowledgeItem[]>('media/mediaKnowledge.json')
    } catch {
      mediaKnowledge.value = []
    }
    try {
      mediaPlans.value = await fetchJson<MediaPlan[]>('media/plans.json')
    } catch {
      mediaPlans.value = []
    }
    try {
      mediaReports.value = await fetchJson<MediaReport[]>('media/reports.json')
    } catch {
      mediaReports.value = []
    }
  }
  ...
  async function fetchDaily(date: string): Promise<DailyPackage | null> {
    try {
      return await fetchJson<DailyPackage>(`daily/${date}.json`)
    } catch {
      return null
    }
  }
  ...
  async function loadShizheng(month: string): Promise<void> {
    try {
      shizheng.value = await fetchJson<ShizhengMonth>(`shizheng/${month}.json`)
    } catch {
      shizheng.value = null
    }
  }
  ...
  async function fetchShizhengMonth(month: string): Promise<ShizhengMonth | null> {
    try {
      return await fetchJson<ShizhengMonth>(`shizheng/${month}.json`)
    } catch {
      return null
    }
  }
  ...
  async function loadShizhengAll(): Promise<void> {
    let months: string[] = []
    try {
      const idx = await fetchJson<{ months: string[] }>('shizheng/index.json')
  ...
  async function loadKnowledge(): Promise<void> {
    try {
      knowledge.value = await fetchJson<KnowledgeItem[]>('shiwu/knowledge.json')
    } catch {
      knowledge.value = []
    }
  }
  ...
  async function loadPinglunIndex(): Promise<void> {
    try {
      pinglunIndex.value = await fetchJson<PinglunEntry[]>('pinglun/index.json')
    } catch {
      pinglunIndex.value = []
    }
  }
  ...
  async function loadPinglunDetail(month: string, id: string): Promise<PinglunDetail | null> {
    try {
      return await fetchJson<PinglunDetail>(`pinglun/${month}/${id}.json`)
    } catch {
      return null
    }
  }
  ...
  async function load(): Promise<void> {
    ...
    error.value = ''
    try {
      const idx = await fetchJson<ContentIndex>('index.json')
